...
const logger = require('../logger')('WordSent');
...
class WordSent {
    ...
    constructor() {
        ...
        this.model_filename = path.resolve(__dirname, './model_9.bin');
    ...
    tag(text) {
        let tokens = tokenizer.tokenize(text).map((token) => {
            return [token, 'X']
        });
